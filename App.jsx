...
const PetalArchiveOS = () => {
  ...
  const monthlyRevenue = [
    { m: 'Jan', r: 45, tag: 'CNY' }, { m: 'Feb', r: 80, tag: 'VAL' }, { m: 'Mar', r: 30 }, { m: 'May', r: 65, tag: 'MOTHER' }, { m: 'Nov', r: 75, tag: 'XMAS' }, { m: 'Dec', r: 100, tag: 'XMAS' }
  ];
  ...
  if (showSuccess) {
    return (
      <div className="min-h-screen bg-[#FDFBF7] flex flex-col items-center justify-center p-6 text-center animate-pulse">
  ...
  return (
    <div className="min-h-screen bg-[#FDFBF7] text-[#1B3022] font-sans p-5 max-w-md mx-auto overflow-x-hidden pb-32">
      ...
      {step === 0 && (
        <div className="pt-10 space-y-6">
          ...
          <div className="bg-white p-8 rounded-[2.5rem] shadow-sm border border-gray-100 space-y-5">
            <div><Label>Event Name</Label><input className="w-full p-4 bg-[#FDFBF7] rounded-2xl border-none outline-none ring-1 ring-gray-50 uppercase text-xs font-bold" placeholder="E.G. 163 MALL APRIL" value={session.eventName} onChange={e => setSession({...session, eventName: toCaps(e.target.value)})} /></div>
            <div>
            ...
            <div className="grid grid-cols-2 gap-4">
               ...
               <div><Label>Booth Fee (RM)</Label><input type="number" className="w-full p-4 bg-[#FDFBF7] rounded-2xl outline-none ring-1 ring-gray-50 text-xs font-bold" placeholder="0" value={session.boothFee} onChange={e => setSession({...session, boothFee: e.target.value})} /></div>
            </div>
        ...
        <div className="space-y-6">
          ...
          {step === 2 && (
            <div className="space-y-6">
              ...
              <section><Label>Price (RM)</Label><input type="number" className="w-full p-4 bg-white border border-gray-100 rounded-2xl outline-none text-3xl font-serif text-[#1B3022] shadow-inner" placeholder="0" value={currentItem.price} onChange={e => setCurrentItem({...currentItem, price: e.target.value})} /></section>
      ...
      {/* 2. DASHBOARD: SESSION STATS & PROFIT */}
        ...
        <div className="space-y-6 pb-20">
          ...
          <div className="grid grid-cols-2 gap-4">
            <div className="bg-white p-6 rounded-[2rem] border border-gray-100 shadow-sm h-36 flex flex-col justify-between">
              ...
              <div className="space-y-2"><StatBar label="TnG" percentage={85} /><StatBar label="Cash" percentage={15} /></div>
            </div>
            <div className="bg-white p-6 rounded-[2rem] border border-gray-100 shadow-sm h-36 flex flex-col justify-between">
               <p className="text-[9px] font-bold text-[#B5935E] uppercase tracking-widest">Profile (Today)</p>
               <div className="space-y-2"><StatBar label="Chinese" percentage={70} /><StatBar label="20-35" percentage={80} color="bg-[#1B3022]"/></div>
            </div>
          </div>
          <section className="bg-white p-8 rounded-[3rem] border border-gray-50 shadow-sm">
            <Label>Daily Best Sellers</Label>
            <div className="space-y-3">
              {[{n:'Necklace',c:8},{n:'Bracelet',c:4}].map((it,i)=>(<div key={i} className="flex justify-between text-xs font-bold uppercase tracking-tighter"><span>{it.n}</span><span className="font-serif italic text-[#B5935E]">{it.c} pcs</span></div>))}
            </div>
          </section>
        </div>
      )}

      {/* 3. HISTORY: BUSINESS BI */}
      {view === 'history' && step > 0 && (
        <div className="space-y-6 pb-20">
          ...
          <section className="bg-white p-8 rounded-[3rem] border border-gray-100 shadow-sm">
            <Label>Yearly Revenue Cycles</Label>
          ...
          <section className="bg-white p-8 rounded-[3rem] border border-gray-100 shadow-sm">
             <Label>Lifetime Top Products</Label>
             <div className="space-y-3"><StatBar label="CZ Necklaces" percentage={60} /><StatBar label="Alphabet Bracelets" percentage={25} /></div>
          </section>
        </div>
      )}
      ...
      {view === 'settings' && step > 0 && (
        <div className="space-y-6 pb-20">
          ...
          <section className="grid grid-cols-2 gap-3">
             <button className="bg-white p-6 rounded-[2rem] border border-gray-100 flex flex-col items-center gap-2 shadow-sm active:scale-95 transition-transform">
               <Database className="text-[#B5935E]" size={24} />
               <span className="text-[9px] font-black uppercase tracking-widest">Master Sheet</span>
             </button>
             <button onClick={() => alert('Price List feature enabled')} className="bg-white p-6 rounded-[2rem] border border-gray-100 flex flex-col items-center gap-2 shadow-sm active:scale-95 transition-transform">
               <FileText className="text-[#B5935E]" size={24} />
               <span className="text-[9px] font-black uppercase tracking-widest">Internal Ref</span>
             </button>
          </section>

          {/* Price Directory Accordion */}
          <section className="bg-white rounded-[2.5rem] border border-gray-100 shadow-sm overflow-hidden">
            <div className="bg-[#1B3022] p-6 text-white font-black text-[10px] uppercase tracking-widest">Bespoke Price List</div>
            <div className="p-4 space-y-2">
              {priceList.map((cat, idx) => (
                <div key={idx} className="border-b border-gray-50 last:border-0">
                  <button onClick={() => setActivePriceCat(activePriceCat === idx ? null : idx)} className="w-full py-4 flex justify-between items-center"><span className="text-[11px] font-black uppercase text-[#1B3022]">{cat.category}</span><ChevronDown size={16} className={`text-[#B5935E] transition-transform ${activePriceCat === idx ? 'rotate-180' : ''}`} /></button>
                  {activePriceCat === idx && (<div className="pb-4 space-y-3">{cat.items.map((it, i) => (<div key={i} className="flex justify-between items-center bg-[#FDFBF7] p-3 rounded-xl border border-gray-50"><span className="text-[10px] font-bold text-gray-400 uppercase">{it.name}</span><span className="text-xs font-serif italic text-[#1B3022]">{it.price}</span></div>))}</div>)}
                </div>
              ))}
            </div>
          </section>
          ...
          <section className="bg-white p-8 rounded-[3rem] border border-gray-100 shadow-sm">
             ...
             <div className="space-y-4">
                <div className="flex justify-between border-b pb-2 text-[10px] font-bold"><span>BOOTH FEE</span><span className="font-serif italic text-[#B5935E]">RM {session.boothFee || 0}</span></div>
                <div className="flex gap-2"><input type="number" className="flex-1 p-3 bg-[#FDFBF7] rounded-xl outline-none text-xs font-bold" placeholder="ADD MISC (E.G. PARKING)..." onChange={(e) => setSession({...session, otherExpenses: session.otherExpenses + Number(e.target.value)})} /><button className="p-3 bg-[#1B3022] text-white rounded-xl"><Plus size={18}/></button></div>
             </div>
          </section>

          <div className="grid grid-cols-2 gap-3">
             <button onClick={() => alert('CSV Exporting...')} className="flex items-center justify-center gap-2 py-4 bg-gray-50 rounded-2xl text-[9px] font-black uppercase text-gray-500"><Download size={14}/> Export CSV</button>
             <button onClick={() => window.location.reload()} className="flex items-center justify-center gap-2 py-4 bg-gray-50 rounded-2xl text-[9px] font-black uppercase text-gray-500"><RefreshCw size={14}/> Refresh</button>
          </div>

          <button onClick={() => setStep(0)} className="w-full bg-red-50 text-red-400 py-6 rounded-3xl font-black text-[10px] uppercase border border-red-100">End Session</button>
        </div>
      )}
