...
const PetalArchiveOS = () => {
  ...
  const stats = useMemo(() => {
    ...
    liveData.forEach(row => {
      ...
      if (row.location) {
        locationRevenue[row.location] = (locationRevenue[row.location] || 0) + price;
      }
  ...
  const logTransaction = async () => {
    ...
    const payload = {
      ...
      session: {
        ...session,
        location: session.location === 'Others' ? session.otherLocation : session.location
      },
      basket,
      customer
    };
  ...
  const basketTotal = useMemo(() => {
    return basket.reduce((acc, item) => acc + Number(item.price || 0), 0);
  }, [basket]);
      ...
      <AnimatePresence mode="wait">
                  ...
                  <div className="mt-3">
                    ...
                    <input
                      ...
                      onChange={e =>
                        setSession(prev => ({
                          ...prev,
                          otherLocation: toCaps(e.target.value)
                        }))
                      }
                    />
                  </div>
        ...
        {view === 'dashboard' && (
          <motion.div key="dash" className="space-y-6">
            ...
            <section className="bg-white p-8 rounded-[3rem] border border-gray-100">
              ...
              <div className="space-y-4">
                <div className="space-y-2">
                  <p className="text-[8px] font-black text-gray-400">Race Split (C/M/I/O)</p>
                  <div className="flex h-2 bg-gray-50 rounded-full overflow-hidden">
                    {Object.entries(stats.raceCounts).map(([race, count], i) => (
                      <div
                        key={race}
                        style={{
                          width: `${(count / (stats.totalPieces || 1)) * 100}%`,
                          backgroundColor: i % 2 === 0 ? '#B5935E' : '#1B3022'
                        }}
                      />
                    ))}
                  </div>
                ...
                <div className="space-y-2">
                  <p className="text-[8px] font-black text-gray-400">Age Split (10s-50s)</p>
                  <div className="flex h-2 bg-gray-50 rounded-full overflow-hidden">
                    {Object.entries(stats.ageCounts).map(([age, count], i) => (
                      <div
                        key={age}
                        style={{
                          width: `${(count / (stats.totalPieces || 1)) * 100}%`,
            ...
            <section className="bg-white p-8 rounded-[3rem] border border-gray-100">
              <Label>Category Performance</Label>
              <div className="space-y-2">
                {Object.entries(stats.categories)
                  ...
                  .map(([cat, count]) => (
                    <div
                      key={cat}
                      className="flex justify-between text-[10px] font-black uppercase border-b border-gray-50 pb-2"
                    >
                      <span>{cat}</span>
                      <span className="text-[#B5935E]">{count} SOLD</span>
                    </div>
                  ))}
              </div>
            </section>
          ...
          <motion.div key="bi" className="space-y-6">
            ...
            <section className="bg-[#1B3022] p-8 rounded-[3rem] text-white shadow-xl relative overflow-hidden">
              ...
              <div className="relative z-10 space-y-4">
                <Label>
                  <span className="text-[#B5935E]">Venue Efficiency</span>
                </Label>
                ...
                <div className="flex justify-between border-t border-white/10 pt-4">
                  <div>
                    <p className="text-[9px] opacity-40 uppercase font-black">Location Total</p>
                    <p className="text-lg font-serif">
                      RM {stats.locationRevenue[session.location]?.toLocaleString() || 0}
                    </p>
                  </div>

                  <div className="text-right">
                    <p className="text-[9px] opacity-40 uppercase font-black">Growth Impact</p>
                    <p className="text-lg font-serif text-[#B5935E]">
                      {((stats.locationRevenue[session.location] / (stats.totalRevenue || 1)) * 100).toFixed(1)}%
                    </p>
                  </div>
                </div>
              </div>
            ...
            <section className="bg-white p-8 rounded-[3rem] border border-gray-100">
              ...
              <div className="space-y-4">
                {Object.entries(stats.locationRevenue).map(([loc, rev]) => (
                  <div key={loc} className="flex justify-between border-b border-gray-50 pb-2 italic">
