...
import { 
  ...
  Activity, DollarSign, Loader2, PieChart, TrendingUp
} from 'lucide-react';
        ...
        {showSuccess && (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 z-[100] flex items-center justify-center bg-[#1B3022]/90 backdrop-blur-md">
            <motion.div initial={{ scale: 0.9 }} animate={{ scale: 1 }} className="bg-white p-12 rounded-[3rem] text-center shadow-2xl">
              <CheckCircle2 className="text-[#1B3022] mx-auto mb-4" size={48} />
        ...
        {step === 0 && (
          <motion.div key="setup" className="pt-6 space-y-6">
            ...
            <div className="bg-white p-7 rounded-[2.5rem] shadow-sm border border-gray-100 space-y-4">
              ...
              <button onClick={() => setStep(1)} className="w-full bg-[#1B3022] text-white py-5 rounded-2xl font-bold shadow-xl">Open Tracker</button>
            </div>
          </motion.div>
        )}
          ...
          <motion.div key="input" className="space-y-6">
            ...
            {step === 1 && (
              <div className="space-y-7">
                ...
                <section><Label>2. Chain Type</Label><div className="grid grid-cols-4 gap-2">{['Cable', 'Snake', 'Paperclip', 'Kiss', 'Bead', 'None', 'Others'].map(ch => (<GridBtn key={ch} label={ch} active={currentItem.chain === ch} onClick={() => setCurrentItem({...currentItem, chain: ch})} />))}</div></section>
                ...
                <button onClick={() => setStep(2)} className="w-full bg-[#1B3022] text-white py-5 rounded-2xl font-black text-sm uppercase">Next Details</button>
              </div>
            )}
            ...
            {step === 2 && (
              <div className="space-y-6">
                ...
                <section><Label>Price (RM)</Label><input type="number" className="w-full p-4 bg-white border border-gray-100 rounded-2xl text-2xl font-serif" value={currentItem.price} onChange={e => setCurrentItem({...currentItem, price: e.target.value})} /></section>
                <div className="flex gap-4"><button onClick={() => setStep(1)} className="flex-1 py-4 text-gray-400 font-bold uppercase text-[10px]">Back</button><button onClick={addToBasket} className="flex-[2] bg-[#B5935E] text-[#1B3022] py-4 rounded-2xl font-black">ADD TO BASKET</button></div>
              </div>
            )}
          ...
          <motion.div key="dash" className="space-y-6">
            ...
            <section className="bg-white p-8 rounded-[3rem] border border-gray-100">
              ...
              <div className="space-y-4">
                <div className="space-y-2"><p className="text-[8px] font-black text-gray-400">Race Split</p>
                ...
                <div className="space-y-2"><p className="text-[8px] font-black text-gray-400">Age Split</p>
        ...
        {view === 'history' && (
          <motion.div key="bi" className="space-y-6">
            ...
            <section className="bg-[#1B3022] p-8 rounded-[3rem] text-white shadow-xl relative overflow-hidden">
               ...
               <div className="relative z-10 space-y-4"><Label><span className="text-[#B5935E]">Venue Comparison</span></Label><h4 className="text-xl font-serif italic">{session.location}</h4><div className="flex justify-between border-t border-white/10 pt-4"><div><p className="text-[9px] opacity-40 uppercase font-black">Total RM</p><p className="text-lg font-serif">RM {stats.locations[session.location]?.toLocaleString() || 0}</p></div><div className="text-right"><p className="text-[9px] opacity-40 uppercase font-black">Vs Overall</p><p className="text-lg font-serif text-[#B5935E]">{((stats.locations[session.location] / (stats.totalRevenue || 1)) * 100).toFixed(1)}%</p></div></div></div>
            </section>
            ...
            <section className="bg-white p-8 rounded-[3rem] border border-gray-100"><Label>Location History</Label>
              <div className="space-y-4">{Object.entries(stats.locations).map(([loc, rev], i) => (<div key={i} className="flex justify-between border-b border-gray-50 pb-2 italic"><span className="text-[9px] font-black uppercase text-gray-400 not-italic">{loc}</span><span>RM {rev.toLocaleString()}</span></div>))}</div>
            </section>
          </motion.div>
        )}
          ...
          <motion.div key="settings" className="space-y-6">
            ...
            <section className="bg-white p-2 rounded-[2.5rem] border border-gray-100 shadow-sm overflow-hidden">
              ...
              <div className="space-y-1">
                {[{c: "Necklaces", i: [{n: "Cable / Snake", p: "95"}, {n: "Paperclip", p: "105"}, {n: "3-Pearl", p: "159"}, {n: "Full Pearl", p: "239"}]},
                  {c: "Bracelets", i: [{n: "Snake / Box", p: "95"}, {n: "M-Paper", p: "105"}, {n: "Charm (3 Charms)", p: "169"}]},
