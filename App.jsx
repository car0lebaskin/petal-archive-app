...
import { 
  ...
  Calendar, Users, CheckCircle2, Package, TrendingUp, Clock, History, LogOut, ShoppingBag, Trash2, LineChart, Layers, FileText, Database, ChevronDown
} from 'lucide-react';
...
const PetalArchiveOS = () => {
  ...
  const [session, setSession] = useState({ 
    ...
    boothFee: '', otherExpenses: 0, staff: ''
  });
  ...
  const endSession = () => {
    if(window.confirm('End Session? This clears ALL data for this bazaar.')) {
      localStorage.clear();
      setSession({ eventName: '', organiser: '', location: '', otherLocation: '', date: new Date().toISOString().split('T')[0], boothFee: '', otherExpenses: 0, staff: '' });
      setBasket([]);
      setStep(0);
      setView('input');
    }
  ...
  const priceList = [
    { 
      ...
      items: [
        ...
        { name: '3-Pearl / 3-Agate / White CZ / Multi CZ', price: 'RM 159' },
    ...
    { 
      ...
      items: [
        ...
        { name: 'M-Paper / Paperclip / Twist Chain', price: 'RM 105' },
        { name: 'Pretzel / CZ Big Link / Knot Chain', price: 'RM 115' },
        { name: 'Special M-Paperclip (W/B/CZ)', price: 'RM 115' },
    ...
    },
    {
      ...
      items: [
        ...
        { name: 'Add-on Floral / STG PDP Charm', price: 'RM 40' },
  ...
  const todayRev = 1420; 
  const currentExpenses = (Number(session.boothFee) || 0) + (Number(session.otherExpenses) || 0);
  const netProfit = todayRev - currentExpenses;
    ...
    <div className="min-h-screen bg-[#FDFBF7] text-[#1B3022] font-sans p-5 max-w-md mx-auto overflow-x-hidden pb-32">
      ...
      {step === 0 && (
        <div className="pt-10 space-y-6">
          ...
          <div className="bg-white p-8 rounded-[2.5rem] shadow-sm border border-gray-100 space-y-5">
            <section><Label>Staff Shift</Label><div className="grid grid-cols-2 gap-2">{['Wife', 'Brother', 'Parent 1', 'Parent 2'].map(s => (<GridButton key={s} label={s} active={session.staff === s} onClick={() => setSession({...session, staff: s})} />))}</div></section>
            ...
            <button onClick={() => setStep(1)} disabled={!session.location || !session.staff} className="w-full bg-[#1B3022] text-white py-5 rounded-2xl font-bold text-lg shadow-xl">Open Tracker</button>
          </div>
        </div>
      )}
        ...
        <div className="space-y-6">
          <div className="flex justify-between items-center bg-[#1B3022] p-4 rounded-2xl shadow-lg sticky top-0 z-10 border border-white/10">
             <div className="flex items-center gap-3 text-white">
               <div className="w-8 h-8 rounded-full bg-[#B5935E] flex items-center justify-center text-[#1B3022] font-black text-[10px]">{session.staff?.charAt(0)}</div>
               <div><p className="text-[9px] font-black uppercase text-[#B5935E]">{basket.length} Items / {session.staff}</p><p className="text-[10px] opacity-50 uppercase tracking-tighter">{session.eventName}</p></div>
             </div>
          ...
          {step === 2 && (
            <div className="space-y-6">
              ...
              <section><Label>Shape</Label><div className="grid grid-cols-3 gap-2">{['Round', 'Oval', 'Rectangle', 'Heart', 'Octagon', 'Others'].map(sh => (<GridButton key={sh} label={sh} active={currentItem.shape === sh} onClick={() => setCurrentItem({...currentItem, shape: sh})} />))}</div>
                {currentItem.shape === 'Others' && <input className="w-full mt-2 p-3 bg-white border border-gray-100 rounded-xl outline-none text-[10px] font-black uppercase" placeholder="SPECIFY SHAPE..." onChange={e => setCurrentItem({...currentItem, otherShape: toCaps(e.target.value)})} />}
              </section>
              <section><Label>Base</Label><div className="grid grid-cols-3 gap-2">{['MOP', 'Black', 'White', 'Clear', 'Others'].map(b => (<GridButton key={b} label={b} active={currentItem.base === b} onClick={() => setCurrentItem({...currentItem, base: b})} />))}</div>
                {currentItem.base === 'Others' && <input className="w-full mt-2 p-3 bg-white border border-gray-100 rounded-xl outline-none text-[10px] font-black uppercase" placeholder="SPECIFY BASE..." onChange={e => setCurrentItem({...currentItem, otherBase: toCaps(e.target.value)})} />}
              </section>
              <section><Label>Colour/Letter</Label><div className="grid grid-cols-3 gap-2">{['Red', 'Blue', 'Yellow', 'Purple', 'Pink', 'Clover', 'White', 'Multi', 'Others'].map(col => (<GridButton key={col} label={col} active={currentItem.colourLetter === col} onClick={() => setCurrentItem({...currentItem, colourLetter: col})} />))}</div>
                {currentItem.colourLetter === 'Others' && <input className="w-full mt-2 p-3 bg-white border border-gray-100 rounded-xl outline-none text-[10px] font-black uppercase" placeholder="SPECIFY COLOUR..." onChange={e => setCurrentItem({...currentItem, otherColour: toCaps(e.target.value)})} />}
              </section>
      ...
      {view === 'dashboard' && step > 0 && (
        <div className="space-y-6 pb-20">
          ...
          <section className="bg-white p-8 rounded-[3rem] border border-gray-100 shadow-sm"><Label>Audience (Today)</Label><div className="grid grid-cols-2 gap-8"><div className="space-y-3"><StatBar label="Female" percentage={92} /><StatBar label="Male" percentage={8} color="bg-[#1B3022]" /></div><div className="space-y-3"><StatBar label="Chinese" percentage={80} /><StatBar label="Malay" percentage={15} /></div></div></section>
        </div>
      )}
        ...
        <div className="space-y-6 pb-20">
          ...
          <section className="bg-[#1B3022] p-8 rounded-[3rem] text-white shadow-xl">
            ...
            <div className="space-y-5">
              {[{loc:'163 Mall',arpd:'1.3k',p:100},{loc:'Waterfront',arpd:'1.0k',p:85}].map((l, i) => (
          ...
          <section className="bg-white p-8 rounded-[3rem] border border-gray-100 shadow-sm"><Label>Yearly Trend</Label>
            <div className="flex items-end gap-1 h-32 pt-4">
              {monthlyRevenue.map((d, i) => (<div key={i} className="flex-1 flex flex-col items-center gap-1 relative">{d.tag && <div className="absolute top-[-15px] text-[6px] bg-[#B5935E] text-[#1B3022] px-1 rounded font-black whitespace-nowrap">{d.tag}</div>}<div className={`w-full rounded-t-sm ${d.r > 70 ? 'bg-[#B5935E]' : 'bg-[#1B3022] opacity-80'}`} style={{ height: `${d.r}%` }} /><span className="text-[7px] font-black text-gray-300 uppercase">{d.m}</span></div>))}
            </div>
          </section>
        </div>
      )}
      ...
      {view === 'settings' && step > 0 && (
        <div className="space-y-6 pb-20">
          ...
          <section className="bg-white p-8 rounded-[3rem] border border-gray-100 shadow-sm"><Label>Expense Logger</Label>
            <div className="flex gap-2"><input type="number" className="flex-1 p-3 bg-[#FDFBF7] rounded-xl outline-none text-xs font-bold" placeholder="ADD BOOTH FEE..." onChange={(e)=>setSession({...session, boothFee: e.target.value})}/><button className="p-3 bg-[#1B3022] text-white rounded-xl"><Plus size={18}/></button></div>
          </section>
